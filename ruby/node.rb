...
class Node
  ...
  def with_socket(domain, protocol, url, options = {})
    ...
    mode = if (options.delete(:bind) ||
               [NNCore::NN_REP, NNCore::NN_BUS, NNCore::NN_PULL].include?(protocol))
      :nn_bind
    else
      :nn_connect
    end
  ...
  def socket_options(sock, options)
    ...
    options.each do |name, value|
      ...
      assert NNCore::LibNanomsg.nn_setsockopt(sock, NNCore::NN_SOL_SOCKET, name, option, 4)
    end
    option.free
  end
